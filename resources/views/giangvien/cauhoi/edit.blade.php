...
@section('title', 'Chỉnh sửa Câu hỏi - ' . $baiKiemTra->tieu_de)
        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('giangvien.lophoc.baikiemtra.cauhoi.update', [$lopHoc->ma_lop, $baiKiemTra->id, $cauHoi->id]) }}" 
                          method="POST" id="editCauHoiForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="loai_cau_hoi" 
                                        id="typeTracNghiem" value="trac_nghiem" {{ $cauHoi->normalizedLoai() === 'trac_nghiem' ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="loai_cau_hoi" 
                                        id="typeTuLuan" value="tu_luan" {{ $cauHoi->normalizedLoai() === 'tu_luan' ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="loai_cau_hoi" 
                                        id="typeDungSai" value="dung_sai" {{ $cauHoi->normalizedLoai() === 'dung_sai' ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="noi_dung" name="noi_dung" rows="4" 
                                      placeholder="Nhập nội dung câu hỏi..." required>{{ old('noi_dung', $cauHoi->noi_dung) }}</textarea>
                        </div>
                        ...
                        <div id="tracNghiemSection">
                            <div class="mb-3">
                                ...
                                <div id="answersContainer">
                                    @php
                                        $answers = $cauHoi->cau_tra_loi_json['lua_chon'] ?? [];
                                        $correctIndex = $cauHoi->cau_tra_loi_json['dap_an_dung'] ?? null;
                                    @endphp
                                    ...
                                    @if(count($answers) > 0)
                                        ...
                                        <div class="input-group mb-2">
                                            <div class="input-group-text">
                                                <input class="form-check-input mt-0" type="radio" name="dap_an_dung" value="{{ $idx }}" {{ $correctIndex === $idx ? 'checked' : '' }}>
                                            </div>
                                            <input type="text" class="form-control" name="lua_chon[]" value="{{ $ans }}" placeholder="Đáp án {{ chr(65 + $idx) }}">
                                        </div>
                                        @endforeach
                                    @else
                                        {{-- Default 4 empty fields if none found --}}
                                        @for($i=0;$i<4;$i++)
                                        <div class="input-group mb-2">
                                            <div class="input-group-text">
                                                <input class="form-check-input mt-0" type="radio" name="dap_an_dung" value="{{ $i }}" {{ $i===0 ? 'checked' : '' }}>
                                            </div>
                                            <input type="text" class="form-control" name="lua_chon[]" placeholder="Đáp án {{ chr(65 + $i) }}">
                                        </div>
                                        @endfor
                                    @endif
                                </div>
                        ...
                        <div id="tuLuanSection" style="display: none;">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="dapAnMau" name="dapAnMau" rows="4" 
                                          placeholder="Nhập đáp án mẫu cho câu hỏi tự luận...">{{ old('dapAnMau', $cauHoi->dap_an ?? '') }}</textarea>
                            </div>
                        </div>
                        ...
                        <div id="dungSaiSection" style="display: none;">
                            <div class="mb-3">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="dap_an" value="dung" id="dapAnDung" {{ (string)($cauHoi->dap_an) === '1' || (string)($cauHoi->dap_an) === 'dung' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="dapAnDung">
                                        <i class="fas fa-check text-success me-1"></i>Đúng
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="dap_an" value="sai" id="dapAnSai" {{ (string)($cauHoi->dap_an) === '0' || (string)($cauHoi->dap_an) === 'sai' ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="diem" name="diem" 
                                           min="0.5" max="10" step="0.5" value="{{ old('diem', $cauHoi->diem) }}" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="thu_tu" name="thu_tu" 
                                           min="0" value="{{ old('thu_tu', $cauHoi->thu_tu) }}">
                                </div>
                            </div>
                        </div>
...
@push('scripts')
<script>
let answerIndex = document.querySelectorAll('input[name="lua_chon[]"]').length || 4;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize visibility based on existing value
    toggleQuestionType();

    // Form client-side validation (light)
    document.getElementById('editCauHoiForm').addEventListener('submit', function(e) {
        const loaiCauHoi = document.querySelector('input[name="loai_cau_hoi"]:checked').value;
